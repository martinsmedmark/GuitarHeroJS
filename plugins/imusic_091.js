	...
	function getMaxUpbeatOffset(tracks){
		...
		for(var trackID in tracks){
			...
			if(track.parts.length){
				...
				offs = Math.min(offs, firstPart.offset, firstPart.pos);
			}
			...
			this.play = function(arg1, nextTime){
				...
				if(self.currentSection && this.parameters.quantize != "off"){
					...
					this.sounds.sort(function(a, b){
						var diffA = timeToQ + a.offset + (-a.offset > timeToQ ? Q : 0); // time to option A in this (or next) bar
						var diffB = timeToQ + b.offset + (-b.offset > timeToQ ? Q : 0); // time to option B in this (or next) bar
						return diffA - diffB;
					});
					console.log(this.sounds.map(s => s.delay));
...
*/
	...
	Data.parseMotif = function(motif, section){
		...
		// posIsDefined -- checks if a position is defined with pos, upbeat or delay 
		// for the whole Motif/Leadin or for any of the child objects (options).
		// Otherwise, use default values
		let posIsDefined = params.upbeat ||params.pos ||params.delay;
		...
		sources.forEach(source => {
			...
			posIsDefined = posIsDefined || src.upbeat || src.pos || src.delay;
...
visualize track objects does not make cut-outs for motifs using the same voice IF triggered after the event or during the cutout period. (a timeWindow problem).
...
Delay: Skapa möjlighet att skicka studsar till utgångar i en viss ordning (fixat i WAXML)
